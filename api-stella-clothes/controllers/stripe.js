...
const retrievePaymentId = async (req, res) => {
  try {
    ...
    if (!sessionId) {
      return res.status(400).send({ error: 'Não existe session ID' });
    }
...
const createCheckoutSession = async (req, res) => {
  ...
  try {
    ...
    await Account.findByIdAndUpdate(
      req.user.id,
      { $set: { 'orderInfo.sessionId': session.id } },
      { new: true }
    );
    ...
    return res.status(500).send({ error: 'Ocorreu um erro ao criar a sessão de pagamento!' });
  }
};
